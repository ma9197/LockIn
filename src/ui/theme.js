...
h2{font:800 18px/1.25 var(--disp);color:var(--ink);margin:32px 0 12px;letter-spacing:-.01em}
h3{font:800 15px/1.3 var(--disp);color:var(--ink)}
p{line-height:1.55}
.cardfoot{display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap;margin-top:16px;padding-top:12px;border-top:1px solid var(--line)}
...
.ph-t h1{font-size:28px;line-height:1.1}
.ph-d{margin:8px 0 0;font-size:14px;line-height:1.5;color:var(--ink2);max-width:60ch}
.ph-a{display:flex;gap:8px;align-items:center;flex-wrap:wrap;padding-top:4px}
.sech{display:flex;align-items:center;gap:12px;margin:32px 0 12px}
.sech h2{margin:0;flex:1;min-width:0}
.sech .tiny{white-space:nowrap}
.secd{margin:-8px 0 12px;font-size:14px;line-height:1.5;color:var(--ink2);max-width:62ch}
...
.cklab{font:600 13px var(--body);color:var(--ink2);margin-top:16px}
.cklab:first-child{margin-top:0}
.item{background:var(--surface2);border:1px solid var(--line2);border-left:4px solid var(--ac,var(--line2));border-radius:12px;padding:16px;margin-top:12px}
.item:first-child{margin-top:0}
.item input,.item select,.item textarea{background:var(--well)}
.item.dim{opacity:.5}
.ihead{display:flex;align-items:center;gap:12px}
.ihead input.nm{flex:1;min-width:0;font-weight:700;padding:12px 12px}
...
.addbtn{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;margin-top:12px;padding:12px;border:1px dashed var(--line2);background:transparent;color:var(--ink2);border-radius:12px;font:700 14px var(--disp)}
.addbtn:hover{border-color:var(--ember);color:var(--ember)}
.togrow{display:flex;align-items:center;gap:16px;padding:12px 0;border-top:1px solid var(--line)}
...
.auth .logo{font-size:32px;margin-bottom:24px;display:block;text-align:center}
.auth .card{padding:24px 20px;margin:0;box-shadow:0 30px 80px #0007}
@media(min-width:600px){.auth .card{padding:32px 28px}}
.auth h1{font-size:24px;margin:0 0 8px;letter-spacing:-.01em}
.auth .lead{color:var(--ink2);font-size:14px;line-height:1.5;margin:0}
.auth .fg{margin-top:20px}
.auth button.pri{width:100%;margin-top:24px;padding:16px;font:800 16px var(--disp);border-radius:12px}
.auth .alt{text-align:center;margin-top:20px;font-size:14px;color:var(--ink2)}
.auth .alt a{font-weight:700}
.auth .foot{text-align:center;margin-top:24px;font-size:13px;color:var(--ink3)}
...
.dh-nav{display:flex;align-items:center;gap:12px;margin-top:16px}
...
.ringrow .ring .val span{font-size:11px}
.ringrow .extra{font-size:11px;margin-top:8px}
.ringbtns{display:flex;justify-content:center;gap:8px;margin-top:8px}
.ringbtns button{min-width:44px;padding:8px 0}
@media(min-width:600px){.ringrow{grid-auto-columns:minmax(150px,1fr)}.ringrow .ring{width:112px;height:112px}.ringrow .ring .val b{font-size:32px}.ringrow .ring .val span{font-size:11px}.ringcard{padding:16px 12px 16px}.ringbtns{gap:12px}.ringbtns button{min-width:56px}}
@media(min-width:900px){.ringrow{grid-auto-flow:row;grid-template-columns:repeat(auto-fit,minmax(108px,1fr));overflow:visible}.ringrow .ring{width:96px;height:96px}.ringrow .ring .val b{font-size:28px}.ringbtns button{min-width:44px}}
...
.wrap{max-width:660px;margin:0 auto;padding:calc(var(--clkh,46px) + 16px) 16px 20px}
.liveclock{position:fixed;top:10px;right:12px;z-index:70;font-variant-numeric:tabular-nums;color:var(--ink);
  display:flex;align-items:center;gap:8px}
.mc-pill{background:var(--surface);border:1px solid var(--line);border-radius:99px;padding:4px 12px;
...
.mc-led small{font-size:.55em;margin-left:4px;opacity:.8}
.mc-flip{display:flex;align-items:center;gap:4px;font-family:var(--disp);font-weight:800}
.mc-flip .fc{background:linear-gradient(180deg,#222A38 48%,#161C28 52%);border:1px solid var(--line2);
  border-radius:8px;padding:4px 8px;position:relative;box-shadow:0 2px 8px rgba(0,0,0,.45)}
.mc-flip .fc:after{content:'';position:absolute;left:0;right:0;top:50%;height:1px;background:rgba(0,0,0,.55)}
.mc-flip .fap{font-size:.6em;color:var(--ink3);margin-left:4px;align-self:flex-end}
.mc-analog{background:var(--surface);border:1px solid var(--line);border-radius:99px;padding:4px;
  display:flex;align-items:center;gap:8px;box-shadow:0 2px 10px rgba(0,0,0,.35)}
.mc-analog .aap{font:800 .62em var(--disp);color:var(--ink3);padding-right:8px}
.mc-ring{background:var(--surface);border:1px solid var(--line);border-radius:99px;padding:4px 12px 4px 4px;
  display:flex;align-items:center;gap:8px;font-weight:800;font-family:var(--disp);box-shadow:0 2px 10px rgba(0,0,0,.35)}
.mc-ring small{color:var(--ink3);font-size:.7em;margin-left:4px}
.card{background:var(--surface);border:1px solid var(--line);border-radius:var(--r);padding:16px;margin:12px 0}
.row{display:flex;align-items:center;gap:12px}.grow{flex:1;min-width:0}.right{margin-left:auto}
.muted{color:var(--ink2);font-size:14px}.tiny{color:var(--ink2);font-size:12px;line-height:1.45}
.num{font-family:var(--disp);font-variant-numeric:tabular-nums}

button{background:var(--surface2);color:var(--ink);border:1px solid var(--line2);border-radius:var(--rm);
  padding:12px 16px;font:700 15px/1.2 var(--body);cursor:pointer;min-height:44px;transition:transform .08s,filter .15s,background .15s,border-color .15s;-webkit-tap-highlight-color:transparent}
button:hover{border-color:var(--ink3)}
button:active{transform:scale(.97)}
button.pri:hover,button.mint:hover{filter:brightness(1.08)}
button.ghost:hover{background:var(--surface2);border-color:transparent}
button.rose:hover{background:#FF5D7314}
button.busy{position:relative;color:transparent!important;pointer-events:none}
button.busy::after{content:'';position:absolute;inset:0;margin:auto;width:16px;height:16px;border-radius:99px;border:2px solid currentColor;border-color:var(--ink) transparent var(--ink) transparent;animation:spin .7s linear infinite}
button.pri.busy::after{border-color:#1A0D05 transparent #1A0D05 transparent}
...
input,select,textarea{background:var(--surface2);color:var(--ink);border:1px solid var(--line2);border-radius:var(--rm);
  padding:12px 12px;font:16px/1.3 var(--body);width:100%;min-height:44px;color-scheme:dark;transition:border-color .15s}
input::placeholder,textarea::placeholder{color:var(--ink3)}
input:hover,select:hover,textarea:hover{border-color:var(--ink3)}
input:focus,select:focus,textarea:focus{outline:2px solid var(--ember);outline-offset:1px;border-color:var(--ember)}
input.err,select.err,textarea.err{border-color:var(--rose)}
input[type=checkbox],input[type=radio],input[type=range],input[type=color]{min-height:0;width:auto}
button:focus-visible,a:focus-visible,[tabindex]:focus-visible,label:focus-within.sw{outline:2px solid var(--ember);outline-offset:2px}
:focus:not(:focus-visible){outline:none}
label.fld{display:block;font:600 13px/1.3 var(--body);color:var(--ink2);margin:12px 0 8px}
label.fld .opt{font-weight:400;color:var(--ink3);margin-left:4px}

.pill{display:inline-flex;align-items:center;gap:8px;padding:4px 12px;border-radius:99px;font:700 12px var(--disp);letter-spacing:.02em;white-space:nowrap}
.chip{display:inline-flex;align-items:center;gap:8px;background:var(--surface2);border:1px solid var(--line2);border-radius:99px;padding:4px 12px;font-size:13px}
...
.grun{display:inline-flex;align-items:center;gap:8px;background:var(--surface);
  border:1px solid #FF6B3555;border-radius:99px;padding:4px 12px;white-space:nowrap;cursor:pointer;
...
@media(max-width:420px){.grun{padding:4px 8px;font-size:11px;gap:4px}}
...
.nav a{flex:0 0 auto;min-width:74px;text-align:center;padding:12px 8px 12px;color:var(--ink3);font:700 11px var(--disp);letter-spacing:.04em;position:relative}
...
/* ---------- tasks ---------- */
.task{display:flex;gap:12px;padding:12px 0;border-bottom:1px solid var(--line);align-items:flex-start;cursor:pointer}
...
.ring .val b{font:800 32px/1 var(--disp);font-variant-numeric:tabular-nums}
.ring .val span{font:700 11px var(--disp);color:var(--ink3);letter-spacing:.08em;margin-top:2px}
.extra{color:var(--ember2);font:800 12px var(--disp);letter-spacing:.06em;text-align:center;margin-top:8px;min-height:15px}
...
.cal-dow{text-align:center;font:700 12px var(--disp);color:var(--ink2);padding:4px 0}
.cell{background:var(--surface);border:1px solid var(--line);border-radius:12px;min-height:64px;padding:8px;
...
.cell .dots{display:flex;gap:4px;margin-top:4px;flex-wrap:wrap}
.cell .gh{font:800 11px var(--disp);color:var(--ember);margin-top:4px;white-space:nowrap}
...
.leg{display:flex;flex-wrap:wrap;gap:8px;margin-top:16px}

/* ---------- friends / queue ---------- */
.req{border:1px solid var(--line);border-radius:var(--r);padding:16px;margin:12px 0;background:var(--surface)}
.qrow{display:flex;align-items:center;gap:12px;padding:8px 0;border-bottom:1px solid var(--line)}
...
.tabbar{display:flex;gap:4px;overflow-x:auto;scrollbar-width:none;background:var(--surface2);border:1px solid var(--line2);
  border-radius:var(--rs);padding:4px;margin:16px 0 4px;-webkit-overflow-scrolling:touch}
.tabbar::-webkit-scrollbar{display:none}
.tabbar button{flex:1 0 auto;border:0;background:transparent;color:var(--ink2);padding:8px 16px;border-radius:8px;
  font:700 13px var(--disp);white-space:nowrap}
.tabbar button.on{background:var(--surface3);color:var(--ember)}
.tabpane{display:none}.tabpane.on{display:block}
.lcrow{display:flex;align-items:center;gap:8px;padding:8px 2px;border-bottom:1px solid var(--line);font-size:13px}
.lcrow:last-child{border-bottom:0}
.diff{font:800 11px var(--disp);border-radius:99px;padding:2px 8px;letter-spacing:.06em}
...
.acitem{display:flex;align-items:center;gap:8px;width:100%;text-align:left;background:none;border:0;
  border-bottom:1px solid var(--line);border-radius:0;padding:12px 12px;font:14px var(--body);color:var(--ink);cursor:pointer}
...
.prow>.hd{display:flex;align-items:center;gap:8px;padding:8px 2px;font-size:13px;cursor:pointer}
...
.ppanel{display:none;padding:4px 2px 16px 20px}
.prow.open .ppanel{display:block}
.pstats{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px}
.pstats .chip{padding:4px 12px;font-size:12px}
.parow{display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid var(--line);font-size:13px}
.parow:last-child{border-bottom:0}
.parow select{width:auto;padding:4px 8px;font-size:12px}

/* ---------- leetcode workspace ---------- */
@media(min-width:1000px){
  .lcgrid{display:grid;grid-template-columns:360px minmax(0,1fr);gap:16px 32px;align-items:start}
  .lcgrid>.full{grid-column:1/-1}
  .lcgrid #noteCard{min-height:430px;cursor:text}
  .stgrid{display:grid;grid-template-columns:1fr 1fr;gap:16px 32px;align-items:start}
}
...
.arrhead{display:flex;align-items:center;gap:12px;margin-bottom:16px}
.arrhead .nm{font:800 15px var(--disp)}
.aptr{display:flex;flex-wrap:wrap;gap:12px;align-items:center;
...
.pchip{display:inline-flex;align-items:center;gap:4px;background:var(--surface3);
  border:1px solid var(--line2);border-radius:99px;padding:4px 8px 4px 4px}
...
.acell input{width:54px;padding:12px 2px;text-align:center;font:700 16px var(--disp);background:var(--surface3);border-color:var(--line2)}
.acell .ix{font:700 12px var(--disp);color:var(--ink3);margin-top:8px}
.acell .pts{min-height:22px;margin-top:8px;display:flex;flex-direction:column;align-items:center;gap:4px}
.ptag{font:800 11px var(--disp);border-radius:5px;padding:2px 8px;line-height:1.5;white-space:nowrap;cursor:pointer}
...
.dayc{background:var(--surface);border:1px solid var(--line);border-radius:var(--r);
  padding:12px 40px 12px 16px;margin:8px 0;cursor:pointer;position:relative}
...
.dayc .dhd{display:flex;align-items:baseline;gap:4px 12px;flex-wrap:wrap}
.dayc .dt{font:800 15px var(--disp);white-space:nowrap}
.dayc .dsum{font:400 13px/1.45 var(--body);color:var(--ink2)}
.dayc .dsum b{font-family:var(--disp);font-weight:800;color:var(--ink)}
.dayc .exp-list{max-height:none;overflow:visible;margin-top:8px}
.dayc.open .exp-list{display:block}
.dayc .lcrow{padding:8px 0;gap:8px}
...
@media(max-width:520px){
  ...
  .dayc .dsum{font-size:12px}
  .dayc .lcrow{gap:4px;font-size:12px}
  .dayc .lcrow .diff{font-size:11px;padding:2px 8px}
  .dayc .lcrow .outb{font-size:11px;padding:2px 4px}
...
.exp-list{display:none;max-height:270px;overflow-y:auto;margin-top:12px;text-align:left}
.stat.open .exp-list{display:block}
.exp-row{display:flex;gap:8px;padding:8px 2px;border-bottom:1px solid var(--line);font-size:13px;align-items:center}
...
.recgrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:12px}
...
.funnel-row{display:grid;grid-template-columns:minmax(64px,34%) 1fr auto;align-items:center;gap:12px;margin:8px 0}
...
.heat{display:grid;grid-template-rows:repeat(7,13px);grid-auto-flow:column;grid-auto-columns:13px;gap:4px}
.heat div{border-radius:3px;background:var(--surface2)}
.rec-chip{display:flex;flex-direction:column;gap:2px;background:var(--surface2);border:1px solid var(--line2);border-radius:12px;padding:12px 16px;min-width:110px}
.rec-chip b{font:800 18px var(--disp)}
.rec-chip span{font:700 11px var(--disp);color:var(--ink2);letter-spacing:.08em;text-transform:uppercase}
...
.chart-tip{position:fixed;background:var(--surface3);border:1px solid var(--line2);border-radius:8px;
  padding:8px 12px;font-size:13px;line-height:1.45;pointer-events:none;z-index:99;display:none;
  box-shadow:0 8px 22px #0008;max-width:230px}
.chart-tip b{font:800 13px var(--disp)}
svg text.cax{font:600 11px var(--mono);fill:var(--ink3)}
svg text.cvl{font:800 11px var(--disp);fill:var(--ink2)}
.hz{cursor:crosshair}
...
.csum .ci span{display:block;font:700 11px var(--disp);color:var(--ink2);letter-spacing:.07em;
  text-transform:uppercase;margin-top:4px;line-height:1.3}
.clegend{display:flex;flex-wrap:wrap;gap:4px 16px;margin-top:12px;align-items:center}
.clegend span{display:flex;align-items:center;gap:8px;font-size:12px;color:var(--ink2)}
...
@media(max-width:560px){
  ...
  .clegend{gap:4px 12px;margin-top:8px}
  .clegend span{font-size:11px}
  svg text.cax{font-size:11px}
  svg text.cvl{font-size:11px}
}
/* finish-line forecast rows */
.pj{padding:12px 0;border-bottom:1px solid var(--line)}
.pj:last-child{border-bottom:0;padding-bottom:2px}
.pjh{display:flex;align-items:center;gap:8px}
.pjh b{font:800 15px var(--disp)}
.pjbar{position:relative;height:13px;border-radius:99px;background:var(--surface2);overflow:hidden;margin:12px 0 8px}
...
.pjm{display:flex;flex-wrap:wrap;gap:4px 16px}
.pjm span{font-size:12px;color:var(--ink3);display:flex;align-items:center;gap:8px}
...
.pjn{margin-top:8px;font-size:13px;line-height:1.5;color:var(--ink2)}
...
.dw{display:flex;flex-direction:column;align-items:center;gap:8px;min-width:0}
.dw .bar{width:100%;height:78px;display:flex;align-items:flex-end;background:var(--surface2);
  border-radius:8px;overflow:hidden}
.dw .bar i{width:100%;display:block;border-radius:8px}
.dw b{font:800 13px var(--disp);font-variant-numeric:tabular-nums}
.dw span{font:700 11px var(--disp);color:var(--ink3);letter-spacing:.05em;text-transform:uppercase}
@media(max-width:560px){
  .dow{gap:4px}
  .dw .bar{height:62px;border-radius:5px}
  .dw b{font-size:12px}
  .dw span{font-size:11px;letter-spacing:0}
  .mixleg span,.mixnote,.pjn{font-size:12px}
...
.stat b{font:800 36px/1.1 var(--disp);font-variant-numeric:tabular-nums;display:block}
...
@media(max-width:560px){
  ...
  .stat span{font-size:11px;letter-spacing:.05em}
  .statgrid{gap:8px}
}
/* comparison rows (getting faster, week vs week): one line on desktop,
   headline on top and the comparison underneath on a phone */
.trow{display:grid;grid-template-columns:1fr auto auto;column-gap:12px;row-gap:1px;
...
.banner{display:flex;align-items:center;gap:12px;flex-wrap:wrap;background:linear-gradient(135deg,#FF6B3518,#FFB34710);
  border:1px solid #FF6B3540;border-radius:var(--r);padding:16px 16px;margin:12px 0;cursor:pointer}
...
.toast{position:fixed;bottom:calc(86px + env(safe-area-inset-bottom));left:50%;transform:translateX(-50%) translateY(20px);
  background:var(--surface3);border:1px solid var(--line2);border-radius:12px;padding:12px 20px;font-weight:700;
  opacity:0;pointer-events:none;transition:all .25s;z-index:99;white-space:nowrap;max-width:92vw}
.toast.show{opacity:1;transform:translateX(-50%) translateY(0)}
.seg-ctl{display:flex;background:var(--surface2);border:1px solid var(--line2);border-radius:var(--rs);padding:4px;gap:4px}
.seg-ctl button{flex:1;border:0;background:transparent;padding:8px 12px;font-size:13px;border-radius:8px;color:var(--ink2)}
.seg-ctl button.on{background:var(--surface3);color:var(--ink)}
.skel{color:var(--ink2);text-align:center;padding:24px 16px;font-size:14px;line-height:1.55}
...
.tl2-dur{position:absolute;top:50%;left:8px;transform:translateY(-50%);font:700 11px var(--disp);
  color:var(--ink2);background:var(--surface);padding:1px 8px;border-radius:6px;white-space:nowrap}
.tl2-body{padding:2px 0 16px}
.tl2-head{display:flex;align-items:center;gap:8px;flex-wrap:wrap;cursor:pointer}
.tl2-head .lab{font-weight:700}
.tl2-sub{font-size:13px;color:var(--ink2);margin-top:4px}
.modchips{display:flex;gap:8px;flex-wrap:wrap;margin-top:8px}
.modchip{display:inline-flex;gap:4px;align-items:center;background:var(--surface2);border:1px solid var(--line2);
  border-radius:8px;padding:4px 8px;font:700 12px var(--disp)}
...
.acc>.acc-h{display:flex;align-items:center;gap:12px;cursor:pointer;-webkit-tap-highlight-color:transparent}
.acc>.acc-b{display:none}
.acc.open>.acc-b{display:block}
.acc.open>.acc-h .tl2-x{transform:rotate(90deg)}
.tl2-item.open .tl2-x{transform:rotate(90deg)}
.tl2-detail{display:none;margin-top:12px}
.tl2-item.open .tl2-detail{display:block}

/* ---------- grind bar ---------- */
.grindbar{background:var(--surface2);border-radius:99px;height:14px;overflow:hidden;position:relative;margin:12px 0 8px}
...
.grind-live .lg-main{display:flex;align-items:flex-start;gap:12px;min-width:0}
.grind-live .lg-fire{font-size:24px;flex:none;line-height:1.15}
...
@media(min-width:640px){
  .grind-live .lg-wrap{flex-direction:row;align-items:center;gap:16px}
...
.splitbar{display:flex;height:52px;border-radius:12px;overflow:hidden;margin:16px 0 4px;touch-action:none}
...
.splitadj{display:flex;align-items:center;gap:8px;font:700 12px var(--disp)}

/* ---------- jobs table ---------- */
.jtable{width:100%;border-collapse:collapse;font-size:14px}
.jtable th{font:600 12px var(--body);color:var(--ink2);text-align:left;padding:8px 12px;border-bottom:1px solid var(--line2)}
.jtable td{padding:12px;border-bottom:1px solid var(--line);vertical-align:top}
...
.blkrow .blk-lab{grid-column:1/-1;font:700 11px var(--disp);color:var(--ink3);letter-spacing:.12em;
  text-transform:uppercase;margin-bottom:-4px}
.blkrow input[type=time]{width:100%;min-width:0;padding:8px 8px;font-size:15px}
.blkrow .blk-x{padding:8px 8px;flex:none}
...
.blockpanel{background:var(--surface2);border:1px solid var(--line2);border-radius:12px;padding:16px;margin-top:12px}
.bp-label{font:600 13px var(--body);color:var(--ink2);margin-bottom:8px}
...
.layout-v2 #dayHeader{background:var(--surface);border:1px solid var(--line);border-radius:var(--r);
  padding:16px 20px 16px;margin-bottom:8px}
.layout-v2 .dh-top{padding-bottom:12px;border-bottom:1px solid var(--line)}
.layout-v2 #phase{border:1px solid currentColor;padding:8px 16px}
...
.layout-v2 .hdr-nav>button{background:var(--surface2);border:1px solid var(--line2);color:var(--ink2);
  width:38px;height:38px;padding:0;border-radius:12px;font-size:18px;flex:none}
...
.backtoday{display:inline-flex;align-items:center;gap:4px;background:#5EA2FF18;border:1px solid #5EA2FF55;
  color:var(--ice);border-radius:99px;padding:4px 12px;margin-left:8px;font:700 12px var(--disp);
...
.layout-v2 .race-track{height:22px;border-radius:8px}
.layout-v2 .cols{margin-top:8px}
.layout-v2 h2{margin:24px 0 12px}
/* both columns start with a heading, so they line up automatically */
.v2only{display:none}
.layout-v2 .v2only{display:block}
.layout-v2 .card{padding:20px}
.layout-v2 .banner{border-radius:var(--r);padding:16px}

/* ---------- quick copy blocks ---------- */
.snipgrid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:16px}
...
.snip{background:var(--surface);border:1px solid var(--line);border-radius:var(--r);padding:20px 16px;cursor:pointer;
...
.snip-v{font-size:13px;color:var(--ink2);margin-top:4px;overflow-wrap:anywhere;max-height:40px;overflow:hidden}
.snip-hint{font:600 13px var(--body);color:var(--ink2);margin-top:12px}
...
.snip-act{width:38px;height:38px;padding:0;display:flex;align-items:center;justify-content:center;flex:none;
  border-radius:12px;background:var(--surface2);border:1px solid var(--line2);color:var(--ink2);font-size:16px}
...
.subrow{display:flex;align-items:center;gap:12px;padding:16px 4px;border-top:1px solid var(--line);cursor:pointer;min-width:0}
...
/* ---------- grind task picker ---------- */
.taskgrid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:16px}
...
.refresh-fab{position:fixed;left:12px;bottom:calc(84px + env(safe-area-inset-bottom));z-index:60;
  ...
  color:var(--ink2);font-size:20px;display:flex;align-items:center;justify-content:center;padding:0;
  box-shadow:0 2px 10px rgba(0,0,0,.4)}
.refresh-fab:active{transform:rotate(180deg) scale(.92)}
@media(min-width:900px){.refresh-fab{left:228px;bottom:16px}}

/* ---------- clock ---------- */
.clockwrap{display:flex;flex-direction:column;align-items:center;gap:8px}
...
@media(min-width:900px){
  ...
  .wrap{max-width:960px;padding:calc(var(--clkh,46px) + 20px) 32px 32px}
  .cols{display:grid;grid-template-columns:1fr 380px;gap:24px;align-items:start}
  .cols>*{min-width:0}
  .cell{min-height:104px;font-size:12px}
  .cell .tchip{display:block;background:var(--surface2);border-radius:5px;padding:2px 8px;margin-top:4px;
...
$('modalHost').innerHTML='<div class="modal-bg"><div class="modal">'
+'<h1 style="font-size:20px">'+(l?'✎ Edit link':'＋ '+lkT().title)+'</h1>'
...
+'<label class="row" style="gap:8px;margin-top:16px;cursor:pointer">'
