...
.story{position:relative;width:340px;max-width:100%;aspect-ratio:9/16;margin:0 auto;border-radius:28px;overflow:hidden;background:var(--bg);border:1px solid var(--line2);box-shadow:0 30px 80px #000a,inset 0 0 0 1px #ffffff08;cursor:pointer;-webkit-tap-highlight-color:transparent}
.st-vp{position:absolute;inset:0;overflow:hidden}
.st-scene{position:absolute;left:0;top:0;width:360px;height:640px;transform-origin:0 0;transition:transform .9s cubic-bezier(.2,.8,.2,1);padding:16px 14px;display:flex;flex-direction:column;gap:12px;background:var(--bg)}
.st-scene .sh1{font:800 20px var(--disp);letter-spacing:-.01em}
.st-scene .sh2{font:800 13px var(--disp);color:var(--ink);margin:2px 0 6px}
.st-scene .card{margin:0;padding:12px;border-radius:12px}
...
.st-tring{position:relative;width:96px;height:96px;flex:none}
.st-tring svg{transform:rotate(-90deg);display:block}
.st-tring .tv{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;font:800 22px var(--disp);font-variant-numeric:tabular-nums}
.st-tring .tv small{font:700 10px var(--disp);color:var(--ink3);letter-spacing:.06em}
.st-btns{display:flex;flex-direction:column;gap:6px;flex:1;min-width:0}
...
.st-note{min-height:78px;align-items:flex-start;font-size:13px;line-height:1.5;flex-wrap:wrap;white-space:pre-wrap}
...
.st-modal{position:absolute;inset:0;background:rgba(5,7,11,.7);backdrop-filter:blur(3px);display:flex;align-items:flex-end;padding:12px;opacity:0;pointer-events:none;transition:opacity .35s;z-index:4}
.st-modal.on{opacity:1}
.st-sheet{background:var(--surface);border:1px solid var(--line2);border-radius:16px;padding:14px;width:100%;transform:translateY(16px);transition:transform .35s cubic-bezier(.2,.8,.2,1)}
...
.st-cur{position:absolute;left:180px;top:320px;width:26px;height:30px;z-index:20;transition:left .75s cubic-bezier(.3,.7,.3,1),top .75s cubic-bezier(.3,.7,.3,1);filter:drop-shadow(0 2px 4px #000a);pointer-events:none}
...
.st-end.on{opacity:1}
.st-end b{font:900 26px/1.1 var(--disp);letter-spacing:-.02em}
...
.st-ff.on{opacity:1}
.ld-story{display:grid;grid-template-columns:minmax(0,1fr);gap:24px;align-items:center;margin-top:28px}
@media(min-width:900px){.ld-story{grid-template-columns:1fr 340px;gap:48px}.ld-story .story{margin:0 0 0 auto}}
.ld-steps{list-style:none;display:grid;gap:8px;margin-top:16px}
...
@media(max-width:599px){.ld-steps{display:none}.ld-story{margin-top:16px}}
`;
...
function fit(){var w=root.clientWidth,h=root.clientHeight;root.style.setProperty('--ss',Math.min(w/360,h/640));}
window.addEventListener('resize',function(){fit();applyT();});fit();
...
var tx=180/z-cx,ty=320/z-cy;
tx=Math.min(0,Math.max(360/z-360,tx));ty=Math.min(0,Math.max(640/z-640,ty));zoomX=tx;zoomY=ty;applyT();}
function sleep(ms){return new Promise(function(r){var t=0;(function tick(){if(run<0)return r();if(!paused&&visible)t+=50;if(t>=ms)return r();setTimeout(tick,50);})();});}
...
function reset(){
zoomTo(null,1);cur.style.left='180px';cur.style.top='330px';
...
var inp=document.getElementById('stInp');zoomTo(document.getElementById('stProb'),1.6);await sleep(700);
await move(inp,-60,0);await click(inp);await type(inp,document.getElementById('stInpT'),'Two Su');
document.getElementById('stAc').classList.add('on');await sleep(350);var ac1=document.getElementById('stAc1');await move(ac1,-40,0);ac1.classList.add('hi');await click(ac1);
...
var tm=document.getElementById('stTimer');zoomTo(tm,1.7);await sleep(700);
...
var rc=document.getElementById('stRc');zoomTo(document.getElementById('stGoals'),1.8);await sleep(800);
...
var note=document.getElementById('stNote');await move(note,-70,-10);await click(note);
...
if('IntersectionObserver' in window){new IntersectionObserver(function(en){en.forEach(function(x){visible=x.isIntersecting;});},{threshold:.2}).observe(root);}
