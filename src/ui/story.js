// Home page "story": a 16:9 frame that plays a scripted walkthrough of one workflow with a visible
// cursor, clicks, typing, zooms and captions. Pure HTML/CSS/JS, no video file. The scene mirrors the
// real LeetCode page (sidebar, header, Solve/Stats tabs, saved tabs row, Problem card, Focus timer and
// Notes side by side), laid out at 960x540 and scaled to the frame; zooms are transforms on the scene.
...
/* main column */
.st-main{padding:16px 22px 14px;min-width:0;display:flex;flex-direction:column;gap:8px;position:relative}
...
/* timer card: ring, then Start / Reset / length, then difficulty */
...
.st-scene .sel{min-height:28px;padding:4px 10px;font:700 11px var(--body);border-radius:9px;background:var(--surface2);border:1px solid var(--line2);display:flex;align-items:center;gap:6px}
...
/* notes card: empty until a problem is picked, then the editor */
...
/* the log sheet, centred like the real modal */
...
.st-sheet{background:var(--surface);border:1px solid var(--line2);border-radius:16px;padding:16px;width:360px;max-width:100%;transform:translateY(14px);transition:transform .35s cubic-bezier(.2,.8,.2,1)}
.st-modal.on .st-sheet{transform:none}
.st-sheet .t{font:800 16px var(--disp);margin-bottom:2px}
.st-sheet .s{font-size:11px;color:var(--ink2);margin-bottom:8px}
...
/* toast, as the real app shows after a log */
...
/* chrome: progress at the top, caption at the bottom, each on its own band */
...
@media(max-width:599px){.ld-steps{display:none}.ld-story{margin-top:16px}.story{border-radius:12px}.st-cap{font-size:11px;padding:7px 10px;bottom:8px;left:8px}.st-bar{left:8px;right:8px;top:6px}.st-bot{height:44px}}
`;

const RING = (r, w, color, dash) => `<svg width="${r * 2}" height="${r * 2}" viewBox="0 0 ${r * 2} ${r * 2}"><circle cx="${r}" cy="${r}" r="${r - w / 2}" fill="none" stroke="var(--surface3)" stroke-width="${w}"/><circle class="p" cx="${r}" cy="${r}" r="${r - w / 2}" fill="none" stroke="${color}" stroke-width="${w}" stroke-linecap="round" stroke-dasharray="${dash}" stroke-dashoffset="0" style="transition:stroke-dashoffset .3s linear"/></svg>`;
const CT = 2 * Math.PI * 50;  // timer ring: r 55, stroke 10 -> radius 50
...
<div class="story" id="story" role="img" aria-label="Animated walkthrough: log a LeetCode solve, write the note">
  <div class="st-top"><div class="st-bar" id="stBar">${[1, 2, 3, 4, 5, 6].map(() => '<i><b></b></i>').join('')}</div></div>
  <div class="st-ff" id="stFF">⏩ 7 min later</div>
  <div class="st-vp"><div class="st-scene" id="stScene">
    <aside class="st-side"><div class="logo">LOCK<em>IN</em> 🔥</div>${NAV.map(([i, l]) => `<a class="${l === 'LeetCode' ? 'on' : ''}">${ic(i, 14)}${l}</a>`).join('')}</aside>
    <main class="st-main">
      ...
      <div class="st-tabs"><span class="on">🧩 Solve</span><span>📊 Stats</span></div>
      ...
      <div class="st-cols">
        ...
        <div><div class="sh2">Notes</div>
          <div class="card st-ncard" id="stNoteCard">
            ...
            <div class="st-ned" id="stNed"><div class="nh"><span class="diff easy">EASY</span><b>Two Sum</b><span class="st-tick" id="stTick">UNSAVED</span></div>
              <div class="inp st-note ph" id="stNote"><span id="stNoteT">What would you tell yourself next time?</span><span class="caret"></span></div>
              <div class="st-code">seen = {}
for i, x in enumerate(nums):
    ...
    <div class="st-cur" id="stCur"><svg width="22" height="26" viewBox="0 0 26 30"><path d="M3 2 L3 24 L9 18.5 L13.5 28 L17.5 26 L13 17 L21 17 Z" fill="#fff" stroke="#111" stroke-width="1.5" stroke-linejoin="round"/></svg></div>
    <div class="st-rip" id="stRip"></div>
  </div></div>
  <div class="st-bot"></div>
  <div class="st-toast" id="stToast">🧩 Two Sum solved · 3 of 3 today · goal hit ✓</div>
...
// Steps shown under the frame on desktop (they light up in sync with the captions).
export const STORY_STEPS = ['Pick the problem', 'Start the focus timer', 'Press Done, log how it went', 'Today counts it', 'Write the note while it is fresh', 'Saved. Next problem.'];
...
var inp=document.getElementById('stInp');inp.className='inp ph';document.getElementById('stInpT').textContent='Type the problem name, matches appear below';document.getElementById('stAc').classList.remove('on');document.getElementById('stAc1').classList.remove('hi');
document.getElementById('stTv').textContent='25:00';ringSet(document.querySelector('#stTimer svg'),CT,1);document.getElementById('stStart').textContent='Start';
...
document.getElementById('stToast').classList.remove('on');
document.getElementById('stNempty').style.display='';document.getElementById('stNed').classList.remove('on');
...
document.getElementById('stModal').classList.remove('on');await sleep(200);if(my!==run)return;
// 4 today counts it
say(4,'Today counts it: 3 of 3, goal hit, streak safe.');bar(4,3400);
...
if(reduced){reset();document.getElementById('stNempty').style.display='none';document.getElementById('stNed').classList.add('on');document.getElementById('stEnd').classList.add('on');}else play();
})();
`;
